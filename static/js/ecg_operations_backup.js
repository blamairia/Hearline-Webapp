...
function initializeECGAnalysis() {
    ...
    // Setup for the conditional ECG protection mechanism
    const trySetupProtection = () => {
        // setupECGSectionProtection will internally check the route
        // and relies on DOM elements being available.
    ...
    if (document.readyState === 'loading') {
        ...
        // DOMContentLoaded has already fired, use a brief timeout
        // to ensure elements are rendered, especially if this script is non-deferred.
...
function checkAndAnalyzeECG() {
    ...
    console.log('ECG check context:', { isEditMode,  });
 ...
 * and the ECG waveform display components, *but only if on an /edit route*.
 */
function protectECGSectionVisibility() {
    if (window.location.pathname.includes('/edit')) {
        // console.log('protectECGSectionVisibility: on /edit route, forcing visibility.');
        forceECGVisibility();
    } else {
        // console.log('protectECGSectionVisibility: not on /edit route, no action.');
    }
}

/**
 * TEMPORARY FIX: Set up mutation observer and interval to watch for ECG section changes,
 * *but only if on an /edit route*.
 */
function setupECGSectionProtection() {
    if (!window.location.pathname.includes('/edit')) {
        console.log('Not on an /edit route. ECG protection mechanism will not be activated.');
        return;
    }

    console.log('On an /edit route. Activating ECG protection mechanism.');

    // Call once at setup to ensure initial visibility
    forceECGVisibility();

    const sectionsToObserve = [
        document.getElementById('ecg-analysis-section'),
        document.getElementById('ecg-waveform-container')
    ];

    const bannerElement = document.querySelector('#ecg-analysis-section > .alert.alert-info');

    const observerCallback = function(mutationsList, observerInstance) {
        // console.log('[ECG Protection] MutationObserver detected changes.');
        let relevantChange = false;
        for (const mutation of mutationsList) {
            if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                // console.log(`[ECG Protection] Style attribute changed on:`, mutation.target);
                // console.log(`[ECG Protection] Old style: ${mutation.oldValue}`);
                // console.log(`[ECG Protection] New style: ${mutation.target.getAttribute('style')}`);
                if (mutation.target.style.display === 'none' || mutation.target.style.visibility === 'hidden' || mutation.target.style.opacity === '0') {
                    relevantChange = true;
                }
            } else if (mutation.type === 'childList' && mutation.target.classList && mutation.target.classList.contains('d-none')) {
                 relevantChange = true;
            }
            // If the banner itself is mutated
            if (bannerElement && mutation.target === bannerElement && mutation.attributeName === 'style') {
                // console.log(`ECG Protection] Banner style attribute changed. Current style: ${bannerElement.getAttribute('style')}`);
                relevantChange = true; // Force re-application if banner style changes
            }
        }
        if (relevantChange) {
            // console.log('[ECG Protection] Relevant change detected, re-forcing visibility.');
            forceECGVisibility();
            if (bannerElement) {
                // console.log(`ECG Protection] Banner style after re-forcing: display=${bannerElement.style.display}, visibility=${bannerElement.style.visibility}, opacity=${bannerElement.style.opacity}`);
            }
        }
    };

    const observerConfig = { attributes: true, childList: true, subtree: true, attributeOldValue: true };

    sectionsToObserve.forEach(section => {
        if (section) {
            const observer = new MutationObserver(observerCallback);
            observer.observe(section, observerConfig);
            // console.log(`ECG Protection] MutationObserver watching`, section);
        } else {
            // console.warn('[ECG Protection] A section to observe was not found.');
        }
    });

    // Specific observer for the banner's style attribute
    if (bannerElement) {
        const bannerObserver = new MutationObserver(function(mutationsList) {
            for(const mutation of mutationsList) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                    const newStyleAttribute = bannerElement.getAttribute('style');
                    console.log(`[ECG Protection - BANNER SPECIFIC] Banner style attribute changed. Old style: "${mutation.oldValue}", New style: "${newStyleAttribute}"`);

                    const currentInlineDisplay = bannerElement.style.display;
                    const currentPropertyValueDisplay = bannerElement.style.getPropertyValue('display');
                    const computedDisplay = window.getComputedStyle(bannerElement).display;

                    if (currentInlineDisplay === 'none' || currentPropertyValueDisplay === 'none' || computedDisplay === 'none') {
                        console.log(`[ECG Protection - BANNER SPECIFIC] Banner detected as hidden (inline style.display: ${currentInlineDisplay}, propertyValue: ${currentPropertyValueDisplay}, computedStyle.display: ${computedDisplay}). Re-forcing visibility IMMEDIATELY.`);

                        // IMMEDIATE, TARGETED FIX for the banner
                        ...
                        bannerElement.classList.remove('d-none'); // Ensure no utility class is hiding it

                        const styleAfterImmediateFix = bannerElement.getAttribute('style');
                        const computedDisplayAfterImmediateFix = window.getComputedStyle(bannerElement).display;
                        console.log(`[ECG Protection - BANNER SPECIFIC] Banner style after IMMEDIATE direct fix (inline): "${styleAfterImmediateFix}", computed display: ${computedDisplayAfterImmediateFix}`);

                        // Also call the general function to ensure other elements are covered and styles are reinforced.
                        forceECGVisibility();
                    }
                }
            }
        });
        bannerObserver.observe(bannerElement, { attributes: true, attributeOldValue: true });
        console.log('[ECG Protection] Specific MutationObserver watching banner style attribute.');
    } else {
        console.warn('[ECG Protection] Banner element for specific observer not found initially.');
    }

    // Fallback interval
    // console.log('[ECG Protection] Setting up setInterval fallback.');
...
function forceECGVisibility() {
    if (!window.location.pathname.includes('/edit')) {
         console.log('forceECGVisibility: Not on /edit page, exiting.');
        return;
    }
    // console.log('[ECG Protection] forceECGVisibility: Running on /edit page.');

    const selectorsToForce = [
        { selector: '#ecg-analysis-section', displayType: 'block' },
        { selector: '#ecg-waveform-container', displayType: 'block' },
        { selector: '#ecg-analysis-results', displayType: 'block' },
        { selector: '#ecg-controls', displayType: 'block' },
        { selector: '#ecg-waveform-chart', displayType: 'block' },
        { selector: '#ecg-analysis-section > .alert.alert-info', displayType: 'block' }, // The banner
        { selector: '#ecg-analysis-loading', displayType: 'block' }, // Consider if this should always be block
        { selector: '#ecg-waveform-loading', displayType: 'block' }, // Consider if this should always be block
        { selector: '#ecg-waveform-error', displayType: 'block' }    // Consider if this should always be block
    ];

    selectorsToForce.forEach(item => {
        const el = document.querySelector(item.selector);
        if (el) {
            // console.log(`[ECG Protection] Forcefully showing ${item.selector}`);
            el.style.setProperty('visibility', 'visible', 'important');
            el.style.setProperty('opacity', '1', 'important');
            el.classList.remove('d-none');
            el.style.setProperty('display', item.displayType, 'important');
            if (item.selector === '#ecg-analysis-section > .alert.alert-info') {
                // console.log(`[ECG Protection] Banner (${item.selector}) style after forcing: display=${el.style.display}, visibility=${el.style.visibility}, opacity=${el.style.opacity}`);
            }
        } else {
            // console.warn(`[ECG Protection] Element not found for selector: ${item.selector} in forceECGVisibility`);
        }
    });
}
